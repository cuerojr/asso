...
const ListadoDeImagenes = ({ imagenes, setControl, control }) => {
  const handleEliminarImagen = (archivoId) => {
    const nuevasImagenes = control.imagenes.filter(
      (img) => img.id !== archivoId
    );
    ...
    <div className="mt-2 container ss">
      ...
      <div className="row">
        {imagenes.map((archivo) => (
          <div
            className="col-xs-12 col-md-3 position-relative mb-4"
            key={archivo.id}
          >
            <div style={{ background: "#6A6A6A" }} className="text-center">
              ...
              />
              <Button
                className="position-absolute remove-image d-flex justify-content-center align-items-center"
                color="danger"
                onClick={() => handleEliminarImagen(archivo.id)}
              >
