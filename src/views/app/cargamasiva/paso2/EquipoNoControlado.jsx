...
const EquipoNoControlado = ({
  ...
  const [cargarOtroMotivoNoControlado, setCargarOtroMotivoNoControlado] =
    useState(false);
    ...
    if (detalleDeComponentesYControles) {
      if (detalleDeComponentesYControles.noControlado === 1) {
        ...
        setearEquipoNoControlado({
          ...
          observacionesMotivosNoControlados:
            detalleDeComponentesYControles.observacionNoControlado,
    ...
    <Fragment>
      <div className="container-fluid m-0 p-0">
        <div className="row mb-2">
          {(equipoSeleccionadoEnCargaMasiva ||
            equipoSeleccionadoCargaIndividual || componenteIndividualSeleccionado) && (
            <Label check className="col-10">
              EQUIPO NO CONTROLADO:
              <div
                className="d-inline ml-2"
                onClick={setControladoNoControlado}
              >
        ...
        <div className="row mb-2 form-equipo-no-controlado">
          <div className="col-12 col-md-8">
            {equipoNoControlado && (
              <InputGroup>
                <InputGroupAddon addonType="prepend">MOTIVO:</InputGroupAddon>
                <Input
                  className="bg-black"
                  type="select"
                  name="motivo"
                  onChange={(e) => {
                    seleccionarMotivoNoControlado(e.target.value);
                    savingData("motivoNoControlado", e.target.value);
                  }}
                  value={motivoNoControlado}
                >
                  <option>SELECCIONAR MOTIVO</option>
                  {motivos.map((motivo, index) => {
                    return (
                      <option key={index} value={motivo.motivo}>
                        {motivo.motivo}
                      </option>
                    );
                  })}
        ...
        {equipoNoControlado && cargarOtroMotivoNoControlado && (
          <div className="row mb-2">
            ...
            <div className="col-12">
              <FormGroup className="mb-0">
                <InputGroup className="borde-gris mt-2">
                  ...
                  <Input
                    className="mt-2 no-border"
                    type="textarea"
                    id="observaciones"
                    value={observacionesMotivosNoControlados}
                    onChange={(e) => {
                      setObservacionesMotivosNoControlados(e.target.value);
                      savingData(
                        "observacionesMotivosNoControlados",
                        e.target.value,
                      );
...
const mapStateToProps = (state) => {
  return {
    ...
    equipoSeleccionadoCargaIndividual:
      state.equiposReducer.equipoSeleccionadoCargaIndividual,
...
export default connect(mapStateToProps, { setearEquipoNoControlado })(
  EquipoNoControlado,
);
