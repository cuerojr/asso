...
import { faFolder, faTrashCan, faFile, faFileCirclePlus } from "@fortawesome/free-solid-svg-icons";
...
const DetalleSeccion = ({
  ...
  const [mostrarModalCantidadInformes, setMostrarModalCantidadInformes] =
    useState(false);
  const [
    mostrarModalConfirmarBorradoSeccion,
    setMostrarModalConfirmarBorradoSeccion,
  ] = useState(false);
  ...
  return (
    <Fragment>
      ...
      <Modal isOpen={mostrarModalCantidadInformes} size="md">
        <ModalBody>
          Esta sección tiene informes cargados, primero debe quitarlos de esta
          sección para poder eliminarla
        </ModalBody>
        <ModalFooter>
          <Button
            color="danger"
            className="btn btn-danger btn-sm btn-outline-danger"
            onClick={() => {
              setMostrarModalCantidadInformes(false);
            }}
          >
            Cerrar
          </Button>
      ...
      <Modal isOpen={mostrarModalConfirmarBorradoSeccion} size="md">
        <ModalBody>Vas a eliminar esta sección. ¿Estás seguro?</ModalBody>
        <ModalFooter>
          <Button
            className="neutro"
            onClick={() => {
              setMostrarModalConfirmarBorradoSeccion(false);
            }}
          >
            Cancelar
          </Button>
      ...
      </Modal>
      {
        <Row>
          <div className="col-md-12">
            <Button color="link" className="pl-0" onClick={volverSecciones}>
              &lt; Secciones
            </Button>
          </div>
          ...
          <div className="col-md-12 d-flex justify-content-between mt-3">
            <div className="custom-control custom-switch hab-seccion">
              ...
              <label className="custom-control-label" htmlFor="customSwitch1">
                {estado ? "Sección habilitada" : "Sección deshabilitada"}
              </label>
            </div>
            <div>
              <Button
              ...
              >
                {" "}
                <FontAwesomeIcon icon={faTrashCan} /> ELIMINAR SECCIÓN
              </Button>
            </div>
          </div>
        </Row>
      ...
      <Row>
        ...
        <div className="col-md-12 mt-3 d-flex">
          <div className="h5">
            <FontAwesomeIcon icon={faFile} /> {seccion.informes} Informes
          </div>
          <div className="ml-3">
            <Button
              color="primary btn-sm"
              onClick={() => {
                abrirModal(seccion.id_empresa, seccion.id);
              }}
            >
              <FontAwesomeIcon icon={faFileCirclePlus} /> NUEVO INFORME
            </Button>
          </div>
        </div>
        <div className="col-md-12 d-flex mt-3">
          <div className="h5">
            <FontAwesomeIcon icon={faFile} /> {seccion.equipos} Equipos
          </div>
          <div className="ml-3">
            <Button
              color="primary btn-sm"
              onClick={() => {
                setModalNuevoEquipo(true);
              }}
            >
              <FontAwesomeIcon icon={faFileCirclePlus} /> NUEVO EQUIPO
            </Button>
          </div>
        </div>
        <NuevoEquipo
          idEmpresa={seccion.id_empresa}
          mostrarModal={modalNuevoEquipo}
          manejarModal={setModalNuevoEquipo}
...
export default connect(mapStateToProps, {
  fetchdeleteSeccion,
  fetchUpdateSeccion /*fetchListarServiciosPorSeccion, fetchlistarInformesEmpresaSeccionServicio, fetchgetDetalleInforme, fetchDeleteInforme*/,
})(DetalleSeccion);
