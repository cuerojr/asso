...
const CargarVariosControles = (props) => {
  ...
  const handleGuardar = () => {
    if (props.equipoNoControlado) {
      guardarEquipoNoControladoIndividual(
        props.equipo.id,
        props.equipoNoControlado.motivoNoControlado,
        props.equipoNoControlado.observacionesMotivosNoControlados,
        props.equipoNoControlado.fechaEquipoNoControlado,
        props.detalleCliente.id,
      ).then((res) => {
        if (res.stat === 1) {
      ...
      const promesas = props.controlesDelEquipo.map((equipo) => {
        const idsDeFallas = equipo.fallasSeleccionada.map(
          (falla) => falla.value,
        );
        if (equipo.estado)
          return altaControl(
            ...
            equipo.estado.id,
            equipo.componente,
            moment(equipo.fecha).format("YYYY-MM-DD"),
            JSON.stringify(idsDeFallas),
            equipo.observacion,
      ...
      Promise.all(promesas)
        .then((respuestas) => {
          if (respuestas[0].stat === 1) {
            NotificationManager.success(
              "Los controles han sido guardados",
              "Hecho",
              3000,
              null,
              null,
            ...
            //props.fetchlistarEquipos(props.clienteSeleccionado);
            props.fetchlistarControles(
              props.clienteSeleccionado,
              props.equipo.id,
              props.equipo.componente,
            );
            props.fetchlistarcomponentes(
              props.clienteSeleccionado,
              props.equipo.id,
            );
            props.fetchModalCargaControles(false);
  ...
  return (
    <Modal
      isOpen={props.modalCargaControles}
      size="lg"
    ...
    >
      ...
      <ModalBody>
        ...
        {props.componentes &&
          props.componentes.map((componente, i) => (
            <CargaControlItem key={i} componente={componente} />
          ))}
      </ModalBody>
    </Modal>
  );
