...
const FallasConsulta = (props) => {
  ...
  const opcionesBarra = {
    ...
    plugins: {
      ...
      title: {
        display: false,
        text: "TENDENCIA DE TIPOS DE FALLAS",
      },
    },
  };

  const distribuirDatos = (datos) => {
    const { resumen_fallas, totales_periodo_lineas, barras_fallas } = datos;

    setResumenFallasFun(transpose(resumen_fallas));
    totales_periodo_lineas.datasets.forEach(
      (element) => (element.backgroundColor = "rgba(255, 255, 255, 0)"),
    );
    setTotalPeriodoLineas(totales_periodo_lineas);
    setBarrasFallas(barras_fallas);
  };

  useEffect(() => {
    if (props.currentTab !== "fallas") return;
  ...
  return (
    <Fragment>
      <Row>
        <Col>
          <h3 className="font-weight-bold">RESUMEN DE FALLAS</h3>
        </Col>
      </Row>
      <Row>
        <Col>
          ...
          >
            <tbody>
              {resumenFallas &&
                resumenFallas.map((tb, index) => {
                  return (
                    <tr
                      key={index}
                      style={{
                        backgroundColor: resumenFallas[index][0],
                        color: pickTextColorBasedOnBgColorSimple(
                          resumenFallas[index][0],
                          "#FFFFFF",
                          "#000000",
                        ),
                      }}
                          ...
                          if (index == 0) {
                            fragmento = (
                              <th
                                className={
                                  indice > 1
                                    ? "color-azul-fondo text-white text-center"
                                    : "color-azul-fondo text-white text-left"
                                }
                                key={indice}
                              >
                                {tab}
                            ...
                            fragmento = (
                              <td key={indice}>
                                ...
                                <div
                                  className={
                                    indice > 1 ? "text-center" : "text-left"
                                  }
      ...
      <Row>
        <Col>
          <MiniTarjeta classes="mr-2 mt-3 ">
            <h3 className="font-weight-bold mt-3 mb-5 color-azul">
              TENDENCIA DE TIPOS DE FALLAS
            </h3>
            {totalPeriodoLineas && (
              <Line
                options={opcionesLinea}
                data={totalPeriodoLineas}
                height={50}
              />
            )}
          </MiniTarjeta>
          ...
          return (
            <Col md="6" key={ind}>
              <MiniTarjeta classes="mr-2 mt-3 textCenter">
                <h3
                  className="font-weight-bold mt-3 mb-5 color-azul"
