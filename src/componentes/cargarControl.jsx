...
const EditarCargarControl = (props) => {
  ...
  useEffect(() => {
    ...
    if (controlIndividualSeleccionado) {
      ...
      let fallasSelecionadasAmostrar = [];
      props.fallas.forEach((falla) => {
        props.controlIndividualSeleccionado.fallas.forEach((fallaControl) => {
          if (falla.id === fallaControl.id) {
            let fallita = {
              color: falla.color,
              value: falla.id,
  ...
  /*const guardarCambios = () => {
    if (props.editar) {
      props.updateControl(
        props.controlSeleccionado.id,
        tipoControlSeleccionado,
    ...
    <Fragment>
      ...
      <Row>
        <div className="col-md-12 d-flex h3 align-items-center justify-content-between">
          ...
          <div className="relative d-flex align-items-center justify-content-end">
            <Dropdown isOpen={dropdownOpen} toggle={toggle}>
              ...
              <DropdownMenu down>
                <DropdownItem
                  onClick={() => props.fetchModalEditarControl(true)}
                >
                  EDITAR
                </DropdownItem>
                <DropdownItem
        ...
        <div className="d-flex align-items-center position-relative">
          {props.controlSeleccionado &&
            props.controlSeleccionado.comentario_cliente && (
              <ComentarioCliente
                comentario={props.controlSeleccionado.comentario_cliente}
              />
            )}
        </div>
        <div className="col-md-12">
