...
import EditarControlModal from "./editarControlModal";
...
const DetalleEquipo = (props) => {
  ...
  const [
    mostrarModalConfirmarEliminarEquipo,
    setMostrarModalConfirmarEliminarEquipo,
  ] = useState(false);
  const [seccion, setSeccion] = useState("");
  const [ruta, setRuta] = useState(props.equipo.ruta);

  const [estado1Control, setEstado1Control] = useState(
    props.equipo.estados["1"].control,
  );
  const [estado1Estado, setEstado1Estado] = useState(
    props.equipo.estados["1"].estado,
  );
  const [estado1Color, setEstado1Color] = useState(
    props.equipo.estados["1"].color,
  );

  const [estado2Control, setEstado2Control] = useState(
    props.equipo.estados["2"].control,
  );
  const [estado2Estado, setEstado2Estado] = useState(
    props.equipo.estados["2"].estado,
  );
  const [estado2Color, setEstado2Color] = useState(
    props.equipo.estados["2"].color,
  );
    ...
    actualizarComponente(
      ...
      if (res.stat == 1) {
        ...
        NotificationManager.success(
          "El componente ha sido Actualizado",
          "Hecho",
          3000,
          null,
          null,
  ...
  const cargarComponente = (nombre, observacion, bajaRPM) => {
    altaComponente(props.equipo.id, nombre, observacion, bajaRPM).then(
      (res) => {
        if (res.stat == 1) {
          props.fetchlistarcomponentes(props.idEmpresa, props.equipo.id);
  ...
  const guardarEquipoNoControlado = () => {
    ...
    guardarEquipoNoControladoIndividual(
      ...
      if (res.stat === 1) {
        NotificationManager.success(
          "El control ha sido actualizado como equipo no Controlado",
          "Hecho",
          3000,
          null,
          null,
  ...
  return (
    <Fragment>
      ...
      <EditarControlModal equipo={props.equipo} />
      {!detalleComponente && (
        <Fragment>
          <Modal isOpen={mostrarModalConfirmarEliminarEquipo} size="md">
            <ModalBody>
              <Row>
                ...
                <Col xs="10">
                  ATENCIÓN: Al borrar un equipo, también borrarás todos los
                  componentes y controles asociados.
                </Col>
              </Row>
            </ModalBody>
            <ModalFooter>
              ...
              <Button
                className="neutro"
                onClick={() => {
                  setMostrarModalConfirmarEliminarEquipo(false);
                }}
              >
                No, cancelar
              </Button>
          ...
          <Row>
            <div className="col-md-12">
              <Button color="link" className="pl-0" onClick={volverEquipos}>
                &lt; Equipos
              </Button>
            </div>
            <div className="col-md-12 d-flex pl-0">
              ...
              {!noControlado && (
                <div style={{ flex: "0 0 170px" }}>
                  <Button color="success" size="sm" onClick={actualizarSeccion}>
                    GUARDAR CAMBIOS
                  </Button>
                </div>
              )}
                ...
                <Fragment>
                  <div style={{ flex: "0 0 170px" }}>
                    <Button
                      color="success"
                      size="sm"
                      onClick={guardarEquipoNoControlado}
                    >
                      GUARDAR CAMBIOS
                    </Button>
                  </div>
          ...
          {!noControlado && (
            <Row>
              <div className="col-md-6 mt-2">
                ...
                <Button
                  onClick={() => {
                    props.fetchModalCargaControles(true);
                  }}
                  className="mt-2"
                >
              ...
              <div className="col-md-6 text-right mt-2">
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    setMostrarModalConfirmarEliminarEquipo(true);
                  }}
                >
                  {" "}
                  <FontAwesomeIcon icon={faTrashCan} /> ELIMINAR EQUIPO
                </Button>
              </div>
            </Row>
          )}
          {!noControlado && (
            <Row className="mt-4">
              <div className="col-md-12">
                ...
                <div className="text-zero top-right-button-container">
                  <Button
                    ...
                    onClick={() => {
                      setModalNuevoComponente(true);
                    }}
                  >
                    {" "}
                    <FontAwesomeIcon icon={faPuzzlePiece} /> NUEVO COMPONENTE
                  </Button>
                </div>
              </div>
              <div className="col-md-12">
              ...
              <div className="col-md-12">
                {props.componentes &&
                  props.componentes.map((item, index) => (
                    <ItemListaComponente
                      item={item}
                      key={index}
              ...
              <NuevoComponente
                mostrarModal={ModalNuevoComponente}
                idEmpresa={props.idEmpresa}
                nombreEmpresa={props.nombreEmpresa}
                nombreEquipo={nombre}
                manejarModal={setModalNuevoComponente}
      ...
      {detalleComponente && (
        <DetalleComponente
          ...
          equipo={props.equipo}
          volverEquipos={volverEquipos}
          salirDetalleComponente={salirDetalleComponente}
