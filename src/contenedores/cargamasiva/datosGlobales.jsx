...
import {
  ...
  setearSeccionesSeleccionadas,
  setearSeccionesSeleccionadasLabel,
  setearFechaGlobal,
...
const DatosGlobales = (props) => {
  const [tituloReferencia, setTituloReferencia] = useState(
    props.titulodeReferencia,
  );
  const [clienteSeleccionado, setClienteSeleccionado] = useState(
    props.clienteSeleccionado,
  );
  const [tipoTesteos, setTipoTesteos] = useState(null);
  const [tipoTesteoSeleccionado, setTipoTesteoSeleccionado] = useState(
    props.tipoDeControlSeleccionado,
  );
  ...
  const seleccionEmpresa = (e) => {
    ...
    props.setClienteSeleccionadoLabel(
  ...
  const seleccionarTipoTesteo = (e) => {
    ...
    props.setTipoDeControlSeleccionadoLabel(
      e.target.options[e.target.selectedIndex].text,
    );
  };
  ...
  const guardar = (next, steps, step) => {
    if (props.cargaMasivaNoFinalizada) {
      props.setearFechaGlobal(
        moment(props.cargaMasivaNoFinalizada.fecha_global_controles),
      );
      modificacionCargaMasiva(
        props.cargaMasivaNoFinalizada.id,
        tituloReferencia,
        clienteSeleccionado,
        props.cargaMasivaNoFinalizada.id_rutas,
        tipoTesteoSeleccionado,
        props.cargaMasivaNoFinalizada.id_secciones,
      ).then((res) => {
        if (res.stat == 1) {
  ...
  useEffect(() => {
    if (props.cargaMasivaNoFinalizada) {
      ...
      props.setTipoDeControlSeleccionado(
        props.cargaMasivaNoFinalizada.id_control,
      );
      ...
      if (
        props.cargaMasivaNoFinalizada.fecha_global_controles === "0000-00-00"
      ) {
        setFecha(moment());
      } else {
  ...
  useEffect(() => {
    ...
    if (props.rutas && props.secciones && props.cargaMasivaNoFinalizada) {
      const rutasIdsDeLaCargaMasiva = JSON.parse(
        props.cargaMasivaNoFinalizada.id_rutas,
      );
      const seccionesDeLaCargaMasiva = JSON.parse(
        props.cargaMasivaNoFinalizada.id_secciones,
      );
      ...
      props.setearRutaSeleccionada(
        JSON.parse(props.cargaMasivaNoFinalizada.id_rutas),
      );
    ...
    if (obj) {
      ...
      el.forEach((seccion) => {
        ...
        tempElement.innerHTML =
          seccion.label.props.dangerouslySetInnerHTML.__html;
  ...
  const handleChangeMultiRuta = (el) => {
    ...
    if (obj) {
      ...
      el.forEach((falla) => {
        ...
        var tempElement = document.createElement("div");
        tempElement.innerHTML =
          falla.label.props.dangerouslySetInnerHTML.__html;
          ...
          <Col>
            <InputGroup className="mb-3">
              ...
              <Input
                ...
                onChange={(e) => setearTituloReferencia(e)}
              />
            </InputGroup>
          </Col>
        </Row>
          ...
          <Col>
            <InputGroup className="mb-3">
              ...
              <Input
                type="select"
                name="cliente"
                onChange={seleccionEmpresa}
                value={clienteSeleccionado}
                disabled={props.deshabilitarCargaSeleccionada}
              >
            ...
            <InputGroup className="mb-3">
              ...
              {props.secciones && (
                <SelectMultiple
                  ...
                  isDisabled={props.deshabilitarCargaSeleccionada}
                />
              )}
            </InputGroup>
          </Col>
          ...
          <Col>
            <InputGroup className="mb-3">
              ...
              {props.rutas && (
                <SelectMultiple
                  items={props.rutas}
                  handleChangeMulti={handleChangeMultiRuta}
                  itemsSeleccionados={rutasParaSelector}
