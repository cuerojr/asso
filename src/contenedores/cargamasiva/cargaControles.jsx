...
const CargaControles = (props) => {
  ...
  useEffect(() => {
    if (equipoSeleccionado != "") {
      props.fetchDetalleDeComponentesYControles(
        props.idCargaMasiva,
        equipoSeleccionado,
      );
  ...
  useEffect(() => {
    if (
      ...
      props.detalleDeComponentesYControles.noControlado == 1
    ) {
      setEquipoControlado(true);
      setMotivoNoControlado(
        props.detalleDeComponentesYControles.motivoNoControlado,
      );
      setObservacionesMotivosNoControlados(
        props.detalleDeComponentesYControles.observacionNoControlado,
      );
    } else {
  ...
  const siguienteEquipoFormulario = (siguienteEquipoDesdeParent) => {
    if (equipoControlado) {
      ...
      guardarEquipoNoControlado(
        props.idCargaMasiva,
        equipoSeleccionado,
        strMotivoNoControlado,
        observacionesMotivosNoControlados,
        moment(fechaEquipoNoControlado).format("YYYY-MM-DD"),
      );
  ...
  const guardar = (
    ...
    props.detalleDeComponentesYControles.componentes.forEach(
      (componente, index) => {
        cargaControlPorComponente(
          props.idCargaMasiva,
          moment(fechas[index]).format("YYYY-MM-DD"),
          estadoSeleccionadoId[index],
          componente.id,
          JSON.stringify(selectedFallasIds[index]),
          observaciones[index],
    ...
    <Fragment>
      {detalleConfirmFinalizarCarga != "" && (
        <ConfirmarFinalizarCarga
          ...
          setDetalleConfirmFinalizarCarga={setDetalleConfirmFinalizarCarga}
          tipoDeControlSeleccionadoLabel={props.tipoDeControlSeleccionadoLabel}
          finalizar={finalizar}
          detalleConfirmFinalizarCarga={detalleConfirmFinalizarCarga}
        />
      )}
      <Tarjeta titulo="">
        <Col className="mt-2">
        ...
        <Row className="mt-4">
          {indiceEquipoSeleccionado != 0 && (
            <Col>
              <p className="mt-2 mb-0">
                {indiceEquipoSeleccionado} de {equipos.length} Equipos
              </p>
            </Col>
          )}
          {indiceEquipoSeleccionado != 0 && (
            <Col className="p-0 text-right pr-3">
      ...
      {cargaFormulario && (
        <Row className="position-relative" style={{ height: "100px" }}>
          <div className="loading" style={{ position: "relative" }} />
        </Row>
      )}
      {!cargaFormulario && equipoSeleccionado !== "" && (
        <Fragment>
          ...
          <FormularioCargaControles
            ...
            siguienteEquipoFormulario={siguienteEquipoFormulario}
            avisarFinalizar={avisarFinalizar}
          />
        </Fragment>
      )}
    </Fragment>
